...
class WechatispApi extends BaseApi {
    ...
    public function pay($request) {
        ...
        if ($types == "JSAPI" && empty($request['isjsapi'])) {
            return [1,
                $request['apihttp'] . "/Pay/jsapi?style=wechatisp&id=" . $request['id'] . '&ddh=' . $request['fxddh'] . '&t=' . time()];
        }

        include_once COMMON_PATH . "/Tool/wechatisp/lib/WxPay.Api.php";
        if ($types == 'JSAPI') {
            include_once COMMON_PATH . "/Tool/wechatisp/WxPay.JsApiPay.php";
            $tools = new \JsApiPay();
            $openId = $tools->GetOpenid($request['peizhi']['wechatisp_subappid'], $request['peizhi']['wechatisp_subappsecret']);
        } else {

        }
        include_once COMMON_PATH . "/Tool/wechatisp/WxPay.NativePay.php";

        \WxPayConfig::setAPPID($request['peizhi']['wechatisp_appid']);
        \WxPayConfig::setMCHID($request['peizhi']['wechatisp_mchid']);
        \WxPayConfig::setSUBAPPID($request['peizhi']['wechatisp_subappid']);
        \WxPayConfig::setSUBMCHID($request['peizhi']['wechatisp_submchid']);
        \WxPayConfig::setKEY($request['peizhi']['wechatisp_key']);
        \WxPayConfig::setAPPSECRET($request['peizhi']['wechatisp_subappsecret']);
    ...
    public function notify($request) {
        include_once COMMON_PATH . "/Tool/wechatisp/lib/WxPay.Api.php";
        include_once COMMON_PATH . "/Tool/wechatisp/lib/WxPay.Notify.php";
        ...
        $jkpz=$jkpz[1];
        \WxPayConfig::setAPPID($jkpz['wechatisp_appid']);
        \WxPayConfig::setMCHID($jkpz['wechatisp_mchid']);
        \WxPayConfig::setSUBAPPID($jkpz['wechatisp_subappid']);
        \WxPayConfig::setSUBMCHID($jkpz['wechatisp_submchid']);
        \WxPayConfig::setKEY($jkpz['wechatisp_key']);
        \WxPayConfig::setAPPSECRET($jkpz['wechatisp_subappsecret']);
    ...
    public function jsapi($request) {
        ...
        switch ($style) {
            case 'wechatisp':
                return $this->jsapi_wechatisp($pzBuffer, $ddBuffer);
                break;
        }
    }

    //微信公众号
    protected function jsapi_wechatisp($pzBuffer, $ddBuffer) {
        //获取支付网关
