...
class StringChange {
    ...
    public function formatMoney($money) {
        ...
        if ($floatPoint[1] == '00')
            $money = round($money, 0);
        else if (preg_match('/[0-9]0/', $floatPoint[1]))
            $money = round($money, 1);
        return $money;
    }
    ...
    public function hiddenUserName($myName) {
        ...
        if (count($arr) > 1) {
            $tmpInt = strlen($arr[0]);
            if ($tmpInt < 3)
                $arr[0] = mb_substr($arr[0], 0, 2, 'UTF-8') . '***';
            else {
        ...
        if (preg_match("/[\x81-\xfe][\x40-\xfe]/", $myName)) {
            ...
            return $strlen == 2 ? $firstStr . '*' : $firstStr . '***' . $lastStr;
        }
        ...
        if ($myName == 'admin')
            ...
            $myName = substr($myName, 0, 2) . '****' . substr($myName, $length, 2);
        }
        return $myName;
    }
