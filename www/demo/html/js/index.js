$(document).ready(function () {
    ...
    $('.mypay').click(function () {
        ...
        //签名
        var sign = hex_md5(qzf_id + ddh + fee + notify_url + qzf_key);

        var index = layer.load();
        layer.msg('正在提交请稍后...');

        var data = {'fxid': qzf_id, 'fxddh': ddh, 'fxdesc': desc, 'fxfee': fee, 'fxattch': attch, 'fxnotifyurl': notify_url, 'fxbackurl': back_url, 'fxpay': pay, 'fxip': ip, 'fxsign': sign, 'fxnoback': 1, 't': Math.random()};
    ...
    if ($('.mybackurl').length > 0) {
        ...
        var mysign = hex_md5(fxstatus + fxid + fxddh + fxfee + qzf_key); //验证签名
        if (mysign == fxsign) {
    ...
    $('.myorderquery').click(function () {
        ...
        var sign = hex_md5(qzf_id + ddh + myaction + qzf_key); //加密
        var data = {'fxid': qzf_id, 'fxddh': ddh, 'fxaction': myaction, 'fxsign': sign, 't': Math.random()};
    ...
    $('.mybalance').click(function () {
        ...
        var myaction = 'money';
        //签名
        var sign = hex_md5(qzf_id + mydate + myaction + qzf_key);

        var index = layer.load();
        var data = {'fxid': qzf_id, 'fxdate': mydate, 'fxaction': myaction, 'fxsign': sign, 't': Math.random()};
